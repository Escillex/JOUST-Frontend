    ...
    <div className="bg-background border border-white/5 rounded-sm overflow-hidden flex flex-col h-full min-h-[400px]">
      ...
      <div className="md:hidden divide-y divide-white/5 p-3 space-y-3">
        {tournaments.length === 0 ? (
          <div className="p-8 text-center text-white/40 text-[10px] uppercase tracking-widest italic">
            No active tournament records found.
          </div>
        ) : (
          tournaments.map((t) => (
            <div key={t.id} className="p-3.5 rounded bg-white/[0.02] border border-white/10 space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-medium text-white truncate">{t.name}</div>
                  <div className="text-[10px] text-white/60 font-mono uppercase tracking-tighter mt-0.5">{t.id.slice(0, 8)}...</div>
                </div>
                <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-sm border shrink-0 ${
              ...
              <div className="flex items-center justify-between gap-3 text-xs">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Link
                    ...
                    className="text-[10px] font-bold uppercase tracking-widest text-white/60 hover:text-white border border-white/10 hover:border-white/30 px-2.5 py-1 rounded-sm transition-all"
                  >
                    Report
                  </Link>
                  {t.status !== 'COMPLETED' && (
                    <button 
                      ...
                      className="text-[10px] font-bold uppercase tracking-widest text-primary hover:text-white hover:bg-primary/20 border border-primary/30 px-2.5 py-1 rounded-sm"
                    >
                      Complete
                    </button>
                  )}
