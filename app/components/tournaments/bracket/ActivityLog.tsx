"use client";
import { LogEntry } from "../../../tournaments/[id]/bracket/types";
const MaximizeIcon = () => (
...
export default function ActivityLog({ logs, onMaximize }: Props) {
  return (
    <div className="bg-foreground/5 border border-foreground/5 p-8 rounded-[3rem]">
      <div className="flex justify-between items-center mb-6 border-b border-foreground/10 pb-4">
        <h3 className="text-xs font-black text-primary uppercase tracking-[0.4em] font-poppins">Activity Log</h3>
      ...
      <div className="bg-background rounded-2xl p-6 h-64 overflow-y-auto no-scrollbar font-mono text-[11px] border border-foreground/5">
        {logs.length === 0 ? (
          <p className="text-foreground/10 uppercase font-black tracking-widest">NO ACTIVITY YET</p>
        ) : (
